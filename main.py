...
async def reset_bot_state(key, iv, region):
    ...
    try:
        ...
        print("✅ Bot state reset - left squad..! [b][i][FFFF00 YT @AKIRA FF   .07@AKIRA FF   .07")
        return True
        ...
        for account_uid, password in accounts_data.items():
            try:
                ...
                if join_packet:
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                    success_count += 1
                    print(f"✅ Join sent from authenticated account: [b][i][FFFF00 YT @ZIKO BOSS   .07@ZIKO BOSS   .07{account_uid}")
...
async def handle_badge_command(cmd, inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        ...
        for i in range(spam_count):
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
            print(f"✅ Sent /{cmd} request #{i+1} with badge[b][i][FFFF00 YT @AKIRA FF   .07@AKIRA FF   .07 {badge_value}")
            await asyncio.sleep(0.1)
        
        success_msg = f"[b][b][b][FFFFFF]✅ Successfully Sent [b][i][FFFF00 YT @AKIRA FF   .07@AKIRA FF   .07{spam_count} Join Requests!\n🎯 Target: {target_uid}\n🏷️ Badge: {badge_value}\n"
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if spam_count >= 150:
            completion_msg = f"[b][b][b][FFFFFF]✅ Reject Spam Completed Successfully for ID [b][i][FFFF00 YT @AKIRA FF   .07@AKIRA FF   .07{target_uid}\n✅ Total packets sent: {spam_count * 2}\n"
        else:
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[C][b][1E90FF]╭━━━━━━━━━━━━━━━━╮
[1E90FF]┃ [00FFFF]YOUTUBE: ZIKO_BOSS@ [1E90FF]
[1E90FF]┃ [00FFFF]INSTAGRAM: ZIKO_BOSS@ [1E90FF]
[1E90FF]┃ [00FFFF]TELEGRAM: ZIKO_BOSS@ [1E90FF]
[1E90FF]┃ [00FFFF]DEVELOPER: ZIKO_COD [1E90FF]
[1E90FF]╰━━━━━━━━━━━━━━━━╯
"""
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                evo_custom_spam_running = False
                                evo_custom_spam_task.cancel()
                                success_msg = f"[b][FFFFFF]✅ SUCCESS! Evolution custom spam stopped successfully! YT @ZIKOO FF   .07\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = """[b][00FFFF]✨ FREE FF GANGAK BOT ✨
                        [00FFFF]WELCOME! SEE COMMANDS BELOW 👇"""
                            ...
                            # Group Commands
                            group_commands = """[C][b][00FFFF]┌── [00FFFF]⚡ اوامر الفريق ⚡
[C][b][00FFFF]├── [FFFFFF]/3  -> [00FFFF]سكواد 3 لاعبين
[C][b][00FFFF]├── [FFFFFF]/5  -> [00FFFF]سكواد 5 لاعبين
[C][b][00FFFF]├── [FFFFFF]/6  -> [00FFFF]سكواد 6 لاعبين
[C][b][00FFFF]├── [FFFFFF]/inv [uid] -> [00FFFF]دعوة الاعب
[C][b][00FFFF]├── [FFFFFF]! [team_code] -> [00FFFF]جلب البوت للفريق
[C][b][00FFFF]├── [FFFFFF]/exit -> [00FFFF]اخراج البوت
[C][b][00FFFF]└── [FFFFFF]/s -> [00FFFF]Start Match"""
                            ...
                            # Advanced Commands
                            advanced_commands = """[C][b][FF00FF]┌── [00FFFF]⚡ أوامر متقدمة  ⚡
[C][b][FF00FF]├── [FFFFFF]/spm_inv [uid] -> [00FFFF]سبام الدعوات (30x)
[C][b][FF00FF]├── [FFFFFF]/stop spm_inv -> [00FFFF]ايقاف سبام الدعوات
[C][b][FF00FF]├── [FFFFFF]/ghost [code] -> [00FFFF]ارسال الاشباح
[C][b][FF00FF]├── [FFFFFF]/lag [code] -> [00FFFF]مقبرة لاغ الفريق
[C][b][FF00FF]├── [FFFFFF]/stop lag -> [00FFFF]ايقاف الاغ
...
[C][b][00FFFF]└── [FFFFFF]/p [uid] [id] [x] -> [00FFFF]Custom Emote (X times)"""
                            ...
                            evo_commands = """[C][b][FF00FF]┌── [00FFFF]⚡ رقصات EMOTES ⚡
...
[C][b][FF00FF]└── [FFFFFF]@sevos -> [00FFFF]Stop Evo Emote Cycle"""
                            ...
                            # AI & Utility Commands
                            ai_commands = """[C][b][00FFFF]┌── [00FFFF]⚡ التحدث مع AI والاوامر الخاصة ⚡
[C][b][00FFFF]├── [FFFFFF]/ai [question] -> [00FFFF]اسأل الذكاء الاصطناعي
[C][b][00FFFF]├── [FFFFFF]/likes [uid] -> [00FFFF]ارسال 100 لايك
[C][b][00FFFF]├── [FFFFFF]/admin -> [00FFFF]معلومات الادمن
[C][b][00FFFF]└── [FFFFFF]/status -> [00FFFF]التحقق من حالة البوت"""
                           ...
                           # Badges command 
                            badge_commands = """[C][b][FF00FF]┌── [00FFFF]⚡سبام دعوات حسب الشارات⚡
[C][b][FF00FF] |     
[C][b][FF00FF]├── [FFFFFF]/s1 [uid] 
-> [00FFFF]سبام شارة ارض الحرف
[C][b][FF00FF]├── [FFFFFF]/s2 [uid] 
-> [00FFFF]سبام شارة V الجديدة
[C][b][FF00FF]├── [FFFFFF]/s3 [uid] 
-> [00FFFF]سبام Badgeالحديثة 
[C][b][FF00FF]├── [FFFFFF]/s4 [uid] 
-> [00FFFF]سبام V الصغيرة
[C][b][FF00FF]├── [FFFFFF]/s5 [uid] 
-> [00FFFF]سبا بادج القديمة"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # Footer
                            footer = """[C][b][1E90FF]╭━━━━━━━━━━━━━━━━╮
[1E90FF]┃ [00FFFF]DEVELOPER: ZIKO TEAM [1E90FF]
[1E90FF]┃ [00FFFF]هدي هديا من ZIKO TEAM الىيكوم [1E90FF]
[1E90FF]╰━━━━━━━━━━━━━━━━╯"""
